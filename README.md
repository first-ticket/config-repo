...
├── application-kafka.yml            # Kafka 공통 설정 (사용하는 서비스만)
...
Kafka를 사용하는 서비스는 `application-kafka.yml` 프로파일을 include하여 공통 Kafka 설정을 적용합니다.
...
`application-kafka.yml`에는 `bootstrap-servers`가 포함되어 있지 않고
환경에 따라 다르기 때문에 서비스별 환경파일에 직접 설정합니다.
